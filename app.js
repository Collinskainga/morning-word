...
const THEMES_ALL = [
  'faith', 'hope', 'strength', 'peace', 'wisdom',
  'love', 'gratitude', 'courage', 'healing', 'purpose',
];

const MILESTONES = [
  { emoji: '🌅', name: 'First light',       desc: 'Read your first verse',  check: s => s.total  >= 1   },
  { emoji: '🌿', name: 'Week of faith',     desc: '7-day streak',           check: s => s.streak >= 7   },
  { emoji: '🕊️', name: 'A faithful month', desc: '30-day streak',          check: s => s.streak >= 30  },
  { emoji: '✨', name: 'Century strong',    desc: '100 verses read',        check: s => s.total  >= 100 },
];
...
function updateNextTime() {
  document.getElementById('home-next-time').textContent = fmtHour(state.hour) + ' tomorrow';
}
...
{"verse":"full verse text","reference":"Book Chapter:Verse (${state.version})","reflection":"2-3 warm sentences connecting the verse to daily life","prayer":"one short closing prayer sentence"}`;
  ...
  try {
    ...
    if (navigator.serviceWorker.controller) {
      navigator.serviceWorker.controller.postMessage({ type: 'CACHE_VERSE', verse: d });
    }
...
function renderDevotional(d, dateOverride) {
  const dateStr = dateOverride || new Date().toLocaleDateString('en-US', {
    weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
  });
...
function copyVerse() {
  ...
  navigator.clipboard.writeText(text).then(() => {
    ...
    if (btn) {
      ...
      setTimeout(() => { btn.textContent = 'Copy verse'; }, 1800);
    }
  });
}
...
function renderHistory() {
  ...
  lbl.textContent = state.history.length + (state.history.length === 1 ? ' verse' : ' verses');
...
function renderStreak() {
  document.getElementById('streak-big').innerHTML        = state.streak + '<span> days</span>';
  ...
  document.getElementById('month-grid').innerHTML = Array.from({ length: daysInMonth }, (_, i) => {
    const day = i + 1;
